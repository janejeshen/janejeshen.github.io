```markdown
---
layout: page
title: Resume
icon: fas fa-file-alt
order: 2
---

# Resume

## Professional Profile

Data professional with experience in data analysis, business intelligence, data visualization, and machine learning. I work with data to uncover insights, support decision-making, and develop practical solutions. I am also expanding my skills in data engineering and modern data technologies.

---

## Education

### Diploma in Data Science & Analytics — Distinction

**Zetech University**  
January 2024 – November 2025  
GPA: 3.72

### Nanodegree in Data Science

**Moringa School**  
July 2022 – February 2023

### Bachelor of Science in Community Health and Development

**Catholic University of Eastern Africa**  
November 2020 – April 2022

---

## Work Experience

### Data Analyst — GOChapaa

**Nairobi, Kenya**  
January 2024 – January 2026

- Analyzed business and operational data in a digital finance and cryptocurrency environment.
- Prepared, cleaned, and organized datasets for analysis and reporting.
- Developed reports that translated complex data into useful business insights.
- Monitored business and performance trends.
- Communicated findings and insights to stakeholders.

### Data Analyst / BI Analyst — Kiondo Market

- Built Power BI dashboards for sales and business performance monitoring.
- Translated business questions into meaningful metrics and reports.
- Analyzed transaction and product data to identify performance trends.
- Improved reporting workflows and supported data-driven decision-making.

### Data Analyst Intern — Kurasa Africa

**Part-time | June 2025 – September 2025**

- Used PostgreSQL and SQL to support reporting workflows.
- Built Power BI dashboards for student engagement, examination outcomes, and institutional reporting.
- Cleaned, transformed, and validated academic datasets.
- Performed exploratory analysis to identify learning trends and performance gaps.
- Supported reporting automation.

---

## Certifications & Training

- Data Science & Analytics — Zetech University
- Data Science Nanodegree — Moringa School
- Ongoing Data Engineering training
- Ongoing Data Science & AI training

---

## Technical Skills

### Programming & Databases

**Python · SQL · T-SQL · PostgreSQL · MySQL · SQL Server · Amazon Redshift**

### Data Analysis & Statistics

**Exploratory Data Analysis · Statistical Analysis · Hypothesis Testing · A/B Testing · Forecasting · KPI Analysis**

### Data Visualization & BI

**Power BI · Advanced Excel · Looker Studio · Tableau · Plotly · Streamlit**

### Data Engineering

**ETL/ELT · Data Pipelines · Data Transformation · Data Validation · Data Warehousing Concepts · dbt · Docker · AWS S3**

### Machine Learning & AI

**XGBoost · LightGBM · Natural Language Processing · LLM Applications**

### Tools

**Git · GitHub · Jupyter · Google Colab · VS Code · Jira**

---

## Professional Links

- [GitHub](https://github.com/janejeshen)
- [LinkedIn](https://www.linkedin.com/in/jane-njuguna)
- [Email](mailto:janenjuguna550@gmail.com)
```
